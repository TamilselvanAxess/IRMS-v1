    ...
    const summary = await Candidate.aggregate([
      {
        $group: {
          ...
          totalLoanAmount: { $sum: "$loanSanctionAmount" },
          totalPaidAmount: { $sum: "$paidAmount" },
          totalPendingAmount: { $sum: "$pendingAmount" },
          totalActiveLoans: { $sum: "$loan" },
          totalInactiveLoans: { $sum: "$loan" },
          totalActiveCandidates: { $sum: "$loan" },
          totalInactiveCandidates: { $sum: "$loan" },
          totalActiveCandidates: { $sum: "$loan" },
          totalInactiveCandidates: { $sum: "$loan" },
          totalActiveCandidates: { $sum: "$loan" },
          totalInactiveCandidates: { $sum: "$loan" },
      ...
      },
      {
        $project: {
          ...
          totalActiveLoans: 1,
          totalInactiveLoans: 1,
          totalActiveCandidates: 1,
          totalInactiveCandidates: 1,
          totalActiveCandidates: 1,
          totalInactiveCandidates: 1,
          totalActiveCandidates: 1,
          totalInactiveCandidates: 1,
...
export const getLoanStatisticsByMonth = async (req, res) => {
    try {
  ...
  export const getLoanCandidatesByMonth = async (req, res) => {
    try {
  ...
  try {
    ...
    loanCandidates.forEach((candidate) => {
      ...
      monthlyStats[monthYear].totalLoanAmount +=
        candidate.loanDistrubutedAmount || 0;
      ...
      data: {
        ...
        categories: categoriesData.reduce((acc, category) => {
          acc[category.category] = {
            ...
            candidates: category.candidates.map((candidate) => candidate._id)
          };
          return acc;
        }, {})
      }
...
export const getCandidatesByFilter = async (req, res) => {
  try {
...
export const getEmployeeDetailsSummary = async (req, res) => {
  try {
    const { id } = req.params;
